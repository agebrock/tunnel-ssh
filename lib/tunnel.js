...
function createConnection(config) {
        ...
        function(error, stream) {
          if (error) {
            ...
            if (!options.silent) {
              ...
              debug('close client:Reason Error');
              client.close();
            }
  ...
  client.tunnel = function(ioStream) {
    debug('creating tunnel..');
    client.createStream({}, function(error, stream) {
      if (error) {
...
function createTunnel(userConfig, callback) {
  ...
  server.listen(userConfig.srcPort, userConfig.srcHost, function(e) {
    ...
    client.createStream({silent: true}, function(error) {
      if (error) {
        return callback(error);
      } else {
...
/*
 ...
 exports.createTunnel = Promise.promisify(createTunnel);
 */
